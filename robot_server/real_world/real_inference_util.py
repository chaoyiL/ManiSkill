...
import numpy as np
from policy.common.pose_repr_util import (
...
def get_real_umi_action(
    ...
    # TODO
    GRIPPER_REPR = "abs" # rel/abs

    n_robots = int(action.shape[-1] // 10)
    env_action = list[Any]()
    for robot_idx in range(n_robots):
        # convert pose to mat
        curr_pose_mat = pose_to_mat(np.concatenate([
        ...
        # TODO:由于当前的夹爪宽度是相对量，所以需要加入转换程序；以后若改成了绝对量再改回来（已经解决）
        if GRIPPER_REPR == "rel":
            curr_gripper = env_obs[f'robot{robot_idx}_gripper_width'][-1]
            # HACK: 增大夹爪运动幅度
            # action_grip = action_grip * 100.0
            action_grip = curr_gripper + action_grip
