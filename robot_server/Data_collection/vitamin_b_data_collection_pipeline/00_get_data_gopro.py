...
class CameraWorker(threading.Thread):
    ...
    def run(self):
        try:
            device_spec = str(self.config.path)
            # Always use OpenCV VideoCapture for reading devices/streams
            ...
            print(f"[CAM] {self.config.name} ready: {self.config.width}x{self.config.height} ({device_spec})")
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    # normalize ram_time to a readable string
                    ram_time_str = datetime.fromtimestamp(ram_time).strftime("%Y.%m.%d_%H.%M.%S.%f")
                    self.queue.put((frame, ram_time_str), block=False)
                except Exception:
                    pass
        except Exception as e:
